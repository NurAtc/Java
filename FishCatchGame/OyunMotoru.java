public class OyunMotoru
{
    ...
    private static int count = 0; //array boyutunu tutuyor //?????????????????????????????????????????????? neden static
	...
	{
	    nesneList[count] = (OyunNesnesi)nesne; //?????????????????????????????????????????????????? neden gelen tip Object
	    count++;
    }
	
	//arrayden gelen indexdeki nesneyi silmek için
	private void nesneSil(int index , OyunNesnesi list[])  //????????????????????????????????????? neden private
	{
		for(int i = index ; i < count-1 ; i++){  //?????????????????? count-1 olmasından emin miyiz
    ...
    public void carpismaKontrol(OyunNesnesi liste[]) //????????????????????????????????????????????????? neden parametre alıyor
	{		
				...
				if((liste[i].xKoordinati == liste[j].xKoordinati) && (liste[i].yKoordinati == liste[j].yKoordinati) && (liste[i].ad != liste[j].ad)){
					if(((liste[i] instanceof Hamsi) && (liste[j] instanceof Yunus)) || ((liste[i] instanceof Yunus) && (liste[j] instanceof Hamsi))){ //???????????????????????????????? hem ad hem class adını kontrol etmeye çok gerek var mıydı
						...
						if(liste[i] instanceof Hamsi){ 
							nesneSil(i,nesneList);   //???????????????????????????????????????????????????? neden liste değil     
							count--;
						}
						if(liste[j] instanceof Hamsi){ //?????????????????????????????????????????????????? else neden değil
					...
					if(((liste[i] instanceof Hamsi) && (liste[j] instanceof Karabatak)) || ((liste[i] instanceof Karabatak) && (liste[j] instanceof Hamsi))){
						...
						if(liste[i] instanceof Hamsi){
							...
							count--;                 //????????????????????????????????????????? bunu neden nesneSil yapmıyor
						}
	...
	public void oyunuBaslat()
	{
		for(int i = 0 ; i < oyunSuresi ; i++)
		{
			...
			for(int j=0; j < count; j++){	
				System.out.println(nesneList[j]);  //??????????????? buradaki println OyunNesnesinin toString'ini çağırmıyor mu
			}
		}
	}
   
   //arraydeki nesneleri istenen formatta yazdýrmak için
   public String toString()                                 //????????????????????????????????????????????? buna ne gerek vardı
   { 
		String result="";
