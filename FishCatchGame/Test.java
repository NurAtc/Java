...
public class Test
{
   public static void main(String[] args)
   {
	  ...
	  for(int i= 0 ; i<10 ; i++){
		...
		random8 = generator.nextInt(5); //???????????????????????????????????????????????????????????????????????? neden 5
		
		Hamsi h = new Hamsi("Hamsi" + (i+1) ,random , random2 , random7 , random8);
		nesne.oyunNesnesiEkle(h); //???????????????????????????? bu fonksiyonu nasıl direk kullandık, static kavramı nasıl
	  }
	  ...
	  random6 = generator.nextInt(16); //???????????????????????????????????????????????????? yanlış değil mi
	  rand6 = random6 + 5;
